# prepare disk
gcloud compute disks create disk-btc-segwit-1 --size=60GB --type=pd-ssd
gcloud compute instances create prepare-disk-instance --machine-type f1-micro
gcloud compute instances attach-disk prepare-disk-instance --disk disk-btc-segwit-1
gcloud compute ssh prepare-disk-instance -- 'sudo mkfs.ext4 -m 0 -F -E lazy_itable_init=0,lazy_journal_init=0,discard /dev/sdb'
gcloud compute instances detach-disk prepare-disk-instance --disk disk-btc-segwit-1
gcloud compute instances delete prepare-disk-instance

# start container
gcloud beta compute instances create-with-container btc-segwit-1 \
  ...
  --disk "name=disk-btc-segwit-1,device-name=disk-btc-segwit-1,mode=rw,boot=no" \
  --tags btc-node
