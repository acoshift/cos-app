# prepare disk
gcloud compute disks create data-btc-1 --size=250GB --type=pd-ssd
gcloud compute instances create prepare-disk-instance --machine-type f1-micro
gcloud compute instances attach-disk prepare-disk-instance --disk data-btc-1
gcloud compute ssh prepare-disk-instance -- 'sudo mkfs.ext4 -m 0 -F -E lazy_itable_init=0,lazy_journal_init=0,discard /dev/sdb'
gcloud compute instances detach-disk prepare-disk-instance --disk data-btc-1
...
gcloud beta compute instances create-with-container btc-1 \
  ...
  --disk "name=data-btc-1,device-name=data-btc-1,mode=rw,boot=no" \
  --tags btc-node
